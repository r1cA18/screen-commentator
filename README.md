...
画面をリアルタイムにキャプチャし、VLMモデルでニコニコ風のスクロールコメントを生成するmacOSアプリです。
...
- ローカルモデル（Ollama）はマシンスペックによってレスポンスのラグが大きく変わります。MacBook Pro M4 / メモリ 64GB でもラグが目立ったため、10 秒程度のレスポンスを求める場合は Gemini API の利用を推奨します。
- Gemini（クラウド）利用時はスクリーンショットがGoogleのAPIに送信されます。画面に機密情報が映っている場合は注意です。利用は自己責任でお願いします！
- 現状は軽量モデル（Qwen3, Gemini 2.5 等）を使用しており、かつプロンプトも適当なので、コメント品質には改善の余地が大幅にあると思っています。
- PR歓迎です！
